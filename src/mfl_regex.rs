...
const fn keyword_string_gen() -> LazyLock<String> {
    LazyLock::new(|| format!(r"(?P<let>{})|(?P<if>{})|(?P<else>{})|(?P<while>{})|(?P<fn>{})", LET_STR, IF_STR, ELSE_STR, WHILE_STR, FN_STR))
}
pub static KEYWORD_STRING : LazyLock<String> = keyword_string_gen();
...
pub static TOKEN_STRING : LazyLock<String> = LazyLock::new(|| {
    let keyword_string: String = keyword_string_gen().to_string();
    format!("(?P<comment>{})|(?P<keyword>{})|(?P<whitespace>{})|(?P<number>{})|(?P<operator>{})|(?P<punctuation>{})|(?P<identifier>{})",COMMENT_STR,keyword_string,WHITESPACE_STR,NUMBER_STR,OPERATOR_STR,PUNCTUATION_STR,IDENTIFIER_STR)
});
pub static TOKEN : LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(TOKEN_STRING.as_str()).unwrap()
});